...
import ShareLinkBox        from '../components/send/ShareLinkBox'
import ReceiverCard        from '../components/send/ReceiverCard'
import SessionSummary      from '../components/send/SessionSummary'
import StatusBadge         from '../components/ui/StatusBadge'
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6 animate-slide-up">
      ...
      <div>
        <h1 className="font-heading font-bold text-3xl text-ink tracking-tight">Send Files</h1>
        <p className="text-ink-muted mt-1 text-sm">
          Select files or a folder — multiple LAN receivers can connect to the same link.
        </p>
      </div>

      {/* Selection card */}
      <div className="card p-6 space-y-5">
        <DropZone onFilesSelected={handleFilesSelected} selectedFiles={selectedFiles} disabled={isLocked} />

        <button onClick={createSession} disabled={!selectedFiles.length || isLocked}
          className="btn-primary w-full py-3.5">
          {phase === 'waiting' ? (
            <><svg className="w-4 h-4 animate-spin" fill="none" viewBox="0 0 24 24">
                <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/>
                <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8H4z"/>
              </svg>Waiting for receivers…</>
          ) : phase === 'active' ? (
            <><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
                  d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"/>
              </svg>{activeCount} receiver{activeCount !== 1 ? 's' : ''} connected</>
          ) : phase === 'done' ? (
            <><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7"/>
              </svg>All done!</>
      ...
      {/* Session summary — shown when all done */}
      {allDone && (
      ...
      {receivers.length > 0 && (
        <div className="space-y-3 animate-slide-up">
          ...
          <div className="space-y-3">
            {receivers.map((r) => (
              <ReceiverCard key={r.socketId} receiver={r} fileList={fileList} totalSize={totalSize} />
            ))}
          </div>
        </div>
      )}
          ...
          <div className="divide-y divide-surface-border max-h-52 overflow-y-auto">
            {fileList.map((f, i) => (
              <div key={i} className="flex items-center gap-3 px-5 py-2.5">
                ...
                <span className="text-xs text-ink truncate flex-1 font-mono" title={f.path}>{f.path}</span>
                <span className="text-xs text-ink-faint font-mono flex-shrink-0">
                  {`${(f.size / 1024).toFixed(1)} KB`}
                </span>
              </div>
